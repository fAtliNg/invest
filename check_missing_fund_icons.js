...
const getIcon = (secid, shortname) => {
  ...
  if (secid && (secid.startsWith('AM') || secid.startsWith('ATON'))) {
    const atonExclusions = ['AMDRUB', 'AMD_RUB__TOM', 'AMD_RUB__TOD'];
    if (!atonExclusions.includes(secid)) return map['ATON'];
  }
  ...
  // General Invest
  if (secid && ['GIPR', 'GICR', 'GIFM'].includes(secid)) return map['GENERAL'];

  // Apriori
  if (secid && ['FLOW'].includes(secid)) return map['APRIORI'];
...
const main = async () => {
    ...
    for (const url of SOURCES) {
        try {
            ...
            if (data.securities && data.securities.data) {
                allBonds = allBonds.concat(data.securities.data);
            }
        } catch (e) {
    ...
    console.log(`Total funds fetched: ${allBonds.length}`);

    const missing = [];
    for (const bond of allBonds) {
        // secid is index 0, shortname is index 1
    const secid = bond[0];
    const shortname = bond[1];
    const icon = getIcon(secid, shortname);
    if (!icon) {
        missing.push({ secid, shortname });
    }
    }

    console.log(`Funds missing icons: ${missing.length}`);
    const fs = require('fs');
    const content = missing.map(m => `${m.secid} | ${m.shortname}`).join('\n');
    fs.writeFileSync('missing_icons.txt', content);
    console.log('List saved to missing_icons.txt');
};
