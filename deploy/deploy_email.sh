...
# deploy_email.sh - Деплой Email сервиса
# Использование: ./deploy/deploy_email.sh [user@host] [-p password]

TARGET=""
PASSWORD=""
PROJECT_DIR="invest"
TAR_NAME="email_src.tar.gz"
LOCAL_DIR=$(pwd)
...
if [ ! -f "$LOCAL_DIR/package.json" ]; then
    ...
    echo "Пример: ./deploy/deploy_email.sh root@1.2.3.4"
    exit 1
fi
...
echo "🚀 Начинаем деплой Email сервиса на $TARGET..."
...
if [ -d "email-service" ]; then
    # Упаковываем содержимое папки email-service
    COPYFILE_DISABLE=1 tar --exclude='node_modules' --exclude='.git' --exclude='dist' --exclude='.DS_Store' --exclude='._*' --exclude='__MACOSX' -czf "$TAR_NAME" -C email-service .
else
    echo "❌ Ошибка: Папка email-service не найдена!"
    exit 1
fi

# 2. Отправка файлов на сервер
echo "📤 Отправка файлов на сервер..."
$SSH_CMD "$TARGET" "mkdir -p ~/$PROJECT_DIR/email-service ~/$PROJECT_DIR/deploy"
$SCP_CMD "$TAR_NAME" "$TARGET:~/$PROJECT_DIR/email-service/"

# Отправляем Dockerfile
$SCP_CMD "deploy/email.Dockerfile" "$TARGET:~/$PROJECT_DIR/deploy/email.Dockerfile"
...
echo '🔄 Пересборка и перезапуск контейнера email-service...'
docker compose up -d --build --force-recreate email-service

echo '⏳ Проверка статуса...'
sleep 5
if docker ps | grep -q \"deploy-email-service-1\"; then
    echo '✅ Контейнер email-service запущен!'
    docker logs --tail 20 deploy-email-service-1
else
    echo '❌ Ошибка: Контейнер email-service не запущен!'
    docker logs deploy-email-service-1
    exit 1
fi
"
...
rm "$TAR_NAME"
echo "✅ Деплой Email сервиса завершен!"
